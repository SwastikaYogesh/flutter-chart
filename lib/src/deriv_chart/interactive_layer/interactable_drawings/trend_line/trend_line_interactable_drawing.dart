...
class TrendLineInteractableDrawing
  ...
  void paintOverYAxis(
    ...
    if (getDrawingState(this).contains(DrawingToolState.selected)) {
      // Lay the two price labels out so equal displayed prices collapse to one
      // label and near-equal ones don't overlap — moving only the dragged
      // point's box while the other stays anchored. The dashed alignment guides
      // in paint() keep pointing at the true prices; only these boxes shift.
      final ValueLabelPairLayout labelLayout = startPoint != null &&
              endPoint != null
          ? layoutValueLabelPair(
          ...
          : const ValueLabelPairLayout(showSecond: true);
      ...
      if (endPoint != null && startPoint != null && labelLayout.showSecond) {
        drawValueLabel(
          canvas: canvas,
          quoteToY: quoteToY,
